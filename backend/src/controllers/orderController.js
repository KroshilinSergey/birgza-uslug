...
exports.createOrder = async (req, res) => {
  try {
    ...
    // Проверяем и преобразуем category в число
    let categoryId = null;
    if (category) {
    ...
    const orderData = {
      ...
      category: categoryId,
      categoryName:
        categoryName || (categoryId ? `Категория ${categoryId}` : null),
      address: address || "",
...
exports.takeOrder = async (req, res) => {
  try {
    ...
    if (order.status !== "new")
      return res
        .status(400)
        .json({ success: false, message: "Заказ уже взят" });
...
exports.completeOrder = async (req, res) => {
  try {
    ...
    if (order.masterId !== masterId) {
      return res
        .status(403)
        .json({ success: false, message: "Вы не можете завершить этот заказ" });
    }
    if (order.status !== "in_progress") {
      return res
        .status(400)
        .json({ success: false, message: "Заказ не в работе" });
    }
...
exports.confirmOrderCompletion = async (req, res) => {
  try {
    ...
    if (order.status !== "awaiting_confirmation") {
      return res
        .status(400)
        .json({ success: false, message: "Заказ не ожидает подтверждения" });
    }
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    if (order.customerId !== userId) {
      return res
        .status(403)
        .json({ success: false, message: "Вы не можете отменить этот заказ" });
    }
    if (order.status !== "new") {
...
exports.getAvailableOrders = async (req, res) => {
  try {
    ...
    if (spec) {
      ...
      // Преобразуем в числа, так как category в БД - число
...
exports.assignMaster = async (req, res) => {
  try {
    ...
    if (order.status !== "new") {
      return res
        .status(400)
        .json({ success: false, message: "Заказ уже не в статусе new" });
    }
    ...
    if (!master || master.role !== "master") {
      return res
        .status(404)
        .json({ success: false, message: "Мастер не найден" });
    }
