...
function normalizeCity(cityName) {
  ...
  if (cityName.includes(',')) return cityName;
  const cleanName = cityName.split(',')[0].trim().toLowerCase();
  const found = STANDARD_CITIES.find(city => city.toLowerCase().startsWith(cleanName));
  return found || `${cityName.split(',')[0].trim()}, регион не указан`;
}

window.STANDARD_CITIES = STANDARD_CITIES;
